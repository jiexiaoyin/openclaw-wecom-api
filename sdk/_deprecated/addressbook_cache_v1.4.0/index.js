...
const log = logger('addressbook_cache');
...
class AddressBookCache {
  ...
  _loadCache() {
    try {
      if (fs.existsSync(this.cachePath)) {
        ...
        log.info('[AddressBookCache] 已加载本地缓存');
        return data;
      }
    } catch (e) {
      log.info('[AddressBookCache] 加载缓存失败:', e.message);
    }
  ...
  _saveCache() {
    try {
      ...
      log.info('[AddressBookCache] 缓存已保存');
    } catch (e) {
      log.info('[AddressBookCache] 保存缓存失败:', e.message);
    }
  }
  ...
  async syncFromAPI(addressbook) {
    log.info('[AddressBookCache] 从 API 同步通讯录...');
    ...
    try {
      ...
      try {
        ...
        log.info(`[AddressBookCache] 获取成员成功: ${users.userlist?.length || 0} 人`);
      } catch (e) {
        log.info('[AddressBookCache] 获取成员失败:', e.message);
      }
      
      this._saveCache();
      log.info(`[AddressBookCache] 同步完成: ${this.cache.departments.length} 部门, ${this.cache.users.length} 员工`);
      
    } catch (e) {
      log.info('[AddressBookCache] 同步失败:', e.message);
    }
  }
   ...
   */
  onUserAdd(user) {
    log.info('[AddressBookCache] 成员新增:', user.name || user.userid);
   ...
   */
  onUserDelete(userId) {
    log.info('[AddressBookCache] 成员删除:', userId);
   ...
   */
  onUserUpdate(user) {
    log.info('[AddressBookCache] 成员更新:', user.userid);
  ...
  onDepartmentAdd(dept) {
    log.info('[AddressBookCache] 部门新增:', dept.name);
  ...
  onDepartmentDelete(deptId) {
    log.info('[AddressBookCache] 部门删除:', deptId);
  ...
  onDepartmentUpdate(dept) {
    log.info('[AddressBookCache] 部门更新:', dept.id);
