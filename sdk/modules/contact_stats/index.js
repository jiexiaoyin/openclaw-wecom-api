...
class ContactStats extends WeComSDK {
  ...
  _loadStats() {
    ...
    try {
      ...
      // B2 (老板 13:34): 用 logger 模块统一, 含 prefix + ISO 时间 + level
      log.warn('_loadStats failed', e);
    }
  ...
  /**
   * 获取成员联系客户统计明细
   * ⚠️ v1.4.0 暂禁：官方文档无 /externalcontact/get_user_client_detail 端点
   * 类似能力请用 getUserBehaviorData（已实现）
   */
  async getUserClientDetail(userId, startDate, endDate) {
    throw new Error('getUserClientDetail 已废弃：官方文档无此端点（v1.4.0），请用 getUserBehaviorData');
    // return this.post('/externalcontact/get_user_behavior_data', {
    //   userid: userId,
  ...
  /**
   * 获取群聊数据统计
   * v1.4.0 修复：原 /externalcontact/get_group_chat_data 错，正确 /externalcontact/groupchat/statistic
   */
  async getGroupChatStat(dayBeginTime, dayEndTime, userIds = [], options = {}) {
    if (!Array.isArray(userIds) || userIds.length === 0) {
      throw new Error('userIds 必须为非空数组（文档要求 owner_filter.userid_list，不能空）');
    }
    if (userIds.length > 100) {
      throw new Error('userIds 最多 100 个');
    }
    // v1.5.2 修复：文档要求 day_begin_time/day_end_time (不是 start_time/end_time)
    //   owner_filter.userid_list[] 是必填，不填时仅可见范围 < 1000 能走
    return this.post('/externalcontact/groupchat/statistic', {
      day_begin_time: dayBeginTime,
      day_end_time: dayEndTime || dayBeginTime,
      owner_filter: { userid_list: userIds },
      order_by: options.orderBy || 1,
      order_asc: options.orderAsc || 0,
  ...
  /**
   * 获取群聊统计详情
   * ⚠️ v1.4.0 暂禁：官方文档无 /externalcontact/get_group_chat_detail 端点
   * 群聊详情请改走 groupchat/get
   */
  async getGroupChatDetail(chatId, startDate, endDate) {
    throw new Error('getGroupChatDetail 已废弃：官方文档无此端点（v1.4.0），请走 groupchat/get');
    // return this.post('/externalcontact/groupchat/statistic_group_by_day', {
    //   chat_id: chatId,
    //   start_time: String(startDate),
    //   end_time: String(endDate)
    // });
  ...
  /**
   * 获取客户流失统计
   * ⚠️ v1.4.0 暂禁：官方文档无 /externalcontact/get_user_lost_data 端点
   */
  async getUserLostStat(startDate, endDate, userId = '') {
    throw new Error('getUserLostStat 已废弃：官方文档无此端点（v1.4.0），建议通过 get_served_list/get_unassigned_list 自助分析');
    // return this.post('/externalcontact/get_user_behavior_data', {
    //   start_time: String(startDate),
    //   end_time: String(endDate),
    //   userid: userId
