...
const { getChangeTypeText } = require('../../utils/change-type');
const { getSpStatusText } = require('../../utils/sp-status');
...
class Callback extends EventEmitter {
  ...
  verifyMessage(msgSignature, timestamp, nonce, encrypt) {
    log.info('[wecom-skill] verifyMessage params:', {
      timestamp,
      nonce,
      encrypt: encrypt ? encrypt.substring(0, 16) + '...' : null,
      signature: msgSignature ? msgSignature.substring(0, 16) + '...' : null
    });
  ...
  async parseMessage(xmlContent) {
    ...
    log.info('[wecom-skill] parseMessage xml keys:', Object.keys(xml));
    log.info('[wecom-skill] parseMessage xml:', JSON.stringify(xml).substring(0, 500));
  ...
  _appendToFile(filename, record) {
    try {
      ...
      const dir = '/opt/openclaw/extensions/wecom-skill/data';
      ...
      log.info('[wecom-skill] 写入文件失败:', e.message);
    }
  }
  ...
  loadEventHistory() {
    try {
      const fs = require('fs');
      const path = require('path');
      const dataDir = '/opt/openclaw/extensions/wecom-skill/data';
      const records = [];
      ...
      for (const filename of files) {
        ...
        if (!fs.existsSync(filePath)) {
          ...
          if (fs.existsSync(oldPath)) {
            ...
            if (raw.startsWith('[')) {
              ...
              log.info('[wecom-skill] 兼容加载旧格式:', oldRecords.length, '条 from', filename);
            }
          }
          continue;
        }
        ...
        log.info('[wecom-skill] 加载', filename, ':', fileRecords.length, '条');
      }

      // 按时间倒序
      records.sort((a, b) => (b.timestamp || 0) - (a.timestamp || 0));
      this.eventHistory = records.slice(0, this.maxHistorySize);
      log.info('[wecom-skill] 事件历史合计:', this.eventHistory.length, '条');
    } catch (e) {
      log.info('[wecom-skill] 加载事件历史失败:', e.message);
    }
  }
  ...
  async handleContactChange(event, record) {
    ...
    // v2026-07-26 10:25 fix: ctMap 抽到 sdk/utils/change-type (SOP-7)
    ...
    record.changeTypeText = getChangeTypeText(ct);
    log.info('[Callback] 通讯录变更: ' + record.changeTypeText + ' UserId=' + record.userId);
  ...
  async handleExternalContactChange(event, record) {
    ...
    // v2026-07-26 10:25 fix: actionMap 抽到 sdk/utils/change-type (SOP-7)
    record.action = getChangeTypeText(changeType, true);
    log.info('[Callback] 外部联系人变更: ' + record.action + ' UserId=' + userId + ' ExternalUserId=' + externalUserId);
  ...
  async handleAddExternalContact(event, record) {
    ...
    log.info('[Callback] 添加外部联系人: UserId=' + record.userId + ' ExternalUserId=' + record.externalUserId);
  ...
  async handleDelExternalContact(event, record) {
    record.userId = event.UserID || event.userid || '';
    record.externalUserId = event.ExternalUserId || event.external_userid || '';
    log.info('[Callback] 删除外部联系人: UserId=' + record.userId + ' ExternalUserId=' + record.externalUserId);
  ...
  async handleEditExternalContact(event, record) {
    record.userId = event.UserID || event.userid || '';
    record.externalUserId = event.ExternalUserId || event.external_userid || '';
    log.info('[Callback] 编辑外部联系人: UserId=' + record.userId + ' ExternalUserId=' + record.externalUserId);
    return { handled: true, userId: record.userId, externalUserId: record.externalUserId };
  }
  ...
  async handleAddHalfExternalContact(event, record) {
    ...
    log.info('[Callback] 添加半程外部联系人: UserId=' + record.userId + ' ExternalUserId=' + record.externalUserId);
    return { handled: true, userId: record.userId, externalUserId: record.externalUserId };
  }
  ...
  async handleDelFollowUser(event, record) {
    record.userId = event.UserID || event.userid || '';
    record.externalUserId = event.ExternalUserId || event.external_userid || '';
    log.info('[Callback] 删除跟进成员: UserId=' + record.userId + ' ExternalUserId=' + record.externalUserId);
    return { handled: true, userId: record.userId, externalUserId: record.externalUserId };
  }
  ...
  async handleTransferFail(event, record) {
    ...
    log.info('[Callback] 客户接替失败: UserId=' + record.userId + ' ExternalUserId=' + record.externalUserId + ' Reason=' + record.failReason);
  ...
  async handleCreateChat(event, record) {
    ...
    log.info('[Callback] 客户群创建: ChatId=' + record.chatId + ' Creator=' + record.creator);
  ...
  async handleUpdateChat(event, record) {
    ...
    log.info('[Callback] 客户群变更: ChatId=' + record.chatId + ' UpdateType=' + record.updateType);
  ...
  async handleDismissChat(event, record) {
    ...
    record.creator = event.Creator || event.creator || '';
    record.createTime = event.CreateTime || event.create_time || 0;
    log.info('[Callback] 客户群解散: ChatId=' + record.chatId + ' Creator=' + record.creator);
  ...
  async handleSubmitApproval(event, record) {
    ...
    log.info('[Callback] 审批提交: ApprovalId=' + approvalId + ' Status=' + record.statusText + ' Submitter=' + submitter);
  ...
  async handleSysApprovalChange(event, record) {
    ...
    record.statusText = this._getApprovalStatusText(spStatus);
    log.info('[Callback] 审批变更: ApprovalId=' + approvalId + ' Status=' + record.statusText);
  ...
  async handleApproval(event, record) {
    ...
    record.spStatus = spStatus;
    record.statusText = this._getApprovalStatusText(spStatus);
    log.info('[Callback] 审批变更(Approval): ApprovalId=' + approvalId + ' Status=' + record.statusText);
    await this._fetchAndSaveApprovalDetail(event, record, 'change');
  ...
  /** 审批状态码转文本 */
  // v2026-07-26 10:25 fix (老板 query B+C): 共享 sp-status.js (SOP-7: 复用 SDK 文档权威 mapping)
  //   修前问题: 11 个值含重复定义 (6/7 与官方不一致) + 用 '状态'+spStatus fallback 显示原始数字
  //   修后: getSpStatusText(7 值 + 友好 fallback)
  _getApprovalStatusText(spStatus) {
    return getSpStatusText(spStatus);
  }
  ...
  async _fetchAndSaveApprovalDetail(event, record, trigger) {
    try {
      ...
      if (spNoList.length === 0) {
        log.info('[Callback] 未找到审批单(' + trigger + ')');
        return;
      }
      ...
      if (detail?.errcode === 0 || detail?.sp_detail) {
        ...
        log.info('[Callback] 审批详情已保存: ' + matchedSpNo);
      } else {
        log.info('[Callback] 获取审批详情失败: ' + (detail?.errmsg || JSON.stringify(detail).slice(0, 100)));
      }
    } catch (e) {
      log.info('[Callback] _fetchAndSaveApprovalDetail 异常: ' + e.message);
    }
  }
  ...
  async handleMeetingStart(event, record) {
    ...
    log.info('[Callback] 会议开始: MeetingId=' + record.meetingId + ' Topic=' + record.topic + ' Host=' + record.hostUserId);
  ...
  async handleMeetingEnd(event, record) {
    ...
    log.info('[Callback] 会议结束: MeetingId=' + record.meetingId + ' Topic=' + record.topic);
  ...
  async handleAddJoinWay(event, record) {
    log.info('[Callback] 添加加入群聊方式:', event);
  ...
  async handleDelJoinWay(event, record) {
    log.info('[Callback] 删除加入群聊方式:', event);
  ...
  async handleKfMsgPush(event, record) {
    log.info('[Callback] 客服消息推送:', event);
  ...
  async handleKfMsgSend(event, record) {
    log.info('[Callback] 客服消息发送:', event);
  ...
  async handleChangeMember(event, record) {
    ...
    // v2026-07-26 10:25 fix: ctMap 抽到 sdk/utils/change-type (SOP-7)
    record.changeTypeText = getChangeTypeText(ct);
    log.info('[Callback] 成员变更: ' + record.changeTypeText + ' UserId=' + record.userId + ' Name=' + record.name);
  ...
  async handleChangeDepartment(event, record) {
    ...
    // v2026-07-26 10:25 fix: ctMap 抽到 sdk/utils/change-type (SOP-7)
    record.changeTypeText = getChangeTypeText(ct);
    log.info('[Callback] 部门变更: ' + record.changeTypeText + ' DeptId=' + record.deptId + ' Name=' + record.name);
  ...
  async handleChangeTag(event, record) {
    ...
    // v2026-07-26 10:25 fix: ctMap 抽到 sdk/utils/change-type (SOP-7)
    record.changeTypeText = getChangeTypeText(ct);
    log.info('[Callback] 标签变更: ' + record.changeTypeText + ' TagId=' + record.tagId + ' TagName=' + record.tagName);
  ...
  async handleMeetingEnded(event, record) {
    log.info('[Callback] 会议结束(历史记录):', event);
  ...
  async handleMeetingParticipantJoin(event, record) {
    ...
    log.info('[Callback] 参会成员加入: MeetingId=' + record.meetingId + ' User=' + record.participantUserId);
  ...
  async handleMeetingParticipantLeave(event, record) {
    ...
    log.info('[Callback] 参会成员离开: MeetingId=' + record.meetingId + ' User=' + record.participantUserId);
  ...
  async handleLivingStatus(event, record) {
    log.info('[Callback] 直播状态变更:', event);
  ...
  async handleChangePsm(event, record) {
    log.info('[Callback] 微盘容量变更:', event);
  ...
  async handleChangeDisk(event, record) {
    log.info('[Callback] 微盘文件变更:', event);
