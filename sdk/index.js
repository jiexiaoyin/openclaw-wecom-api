...
// v2026-07-26 21:55 老板 query "BDE" 摸清的 4 个新 builder (Date/Contact/Location/RelatedApproval)
const attachExtraValueBuilders = require('./modules/approval/value-builders-extra');
...
const KF = require('./modules/kf');
const Misc = require('./modules/misc');
const Permission = require('./core/permission');

const WeComSDK = require('./sdk');

class WeComPlugin extends WeComSDK {
  constructor(config) {
    super(config);
    this.config = config;
    ...
    // attach 4 个新 builder (per v2026-07-26 21:55 老板 query "BDE")
    attachExtraValueBuilders(Approval);
    ...
    this.journal = new Journal(config);
    ...
    // v1.5.2: msgaudit 已存在但漏挂, 加 this.msgaudit = new (Mtgaudit)(config)
    const Msgaudit = require('./modules/msgaudit');
    this.msgaudit = new Msgaudit(config);
    ...
    // v1.5.2 coverage-enhance: misc 模块 (card/corpgroup/wedrive/exmail 等低频端点)
    this.misc = new Misc(config);
    this.kf = new KF(config);
  ...
  getModules() {
    return [
      ...
      'thirdparty', 'moments', 'oceanengine', 'messenger', 'contactstats', 'msgaudit', 'sensitive', 
