...
const { verifyWecomSignature, decryptWecomEncrypted, encryptWecomPlaintext, computeWecomMsgSignature } = require('../sdk/crypto');
...
async function testCrypto() {
  ...
  assert.strictEqual(decrypted, plaintext, 'encrypt/decrypt round-trip must match');
  ...
  assert.strictEqual(isValid, true, 'signature verification must pass');
  ...
  assert.strictEqual(typeof sig, 'string', 'signature must be string');
